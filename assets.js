...
const wsFunctions = "<script>var HOST = location.origin.replace(/^http/, 'ws');var wss = new WebSocket(HOST);wss.onmessage = function (event) {  var msg = JSON.parse(event.data); if (msg.boardId === boardId) {$('#nextPicture').html(msg.next);}}</script>";
